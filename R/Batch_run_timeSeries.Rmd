...
# reference year for the time series: 1995-2014 - change in HelperScript get_timeSeries().
...
# ADDED
yearmonth1<-rep(NA, length(mod_gfdl))
yearmonth2<-rep(NA, length(mod_gfdl))
yearmonth1_fut<-rep(NA, length(mod_gfdl))
yearmonth2_fut<-rep(NA, length(mod_gfdl))
...
# ADDED 1 to yearmonth

# hist
directory_zoom<-paste0(directory, "ZooMSS/_tmp/historical/")
filename<-"zoomss_gfdl-esm2m_nobc_historical_nosoc_co2_tcb_global_annual_1950_2005.nc4"
hist<-extractFishCDF(directory_zoom, filename , variable_to_extract[1], yearmonth1[1], yearmonth2[1], average_whole_period = FALSE)

# 126
directory_zoom<-paste0(directory, "ZooMSS/_tmp/future/")
filename<-"zoomss_gfdl-esm2m_nobc_rcp26_nosoc_co2_tcb_global_annual_2006_2100.nc4"
fut126<-extractFishCDF(directory_zoom, filename , variable_to_extract[1], yearmonth1[1], yearmonth2[1], average_whole_period = FALSE)

# 585
filename<-"zoomss_gfdl-esm2m_nobc_rcp85_nosoc_co2_tcb_global_annual_2006_2100.nc4"
fut585<-extractFishCDF(directory_zoom, filename , variable_to_extract[1], yearmonth1[1], yearmonth2[1], average_whole_period = FALSE)
...
# save(data_ipsl_CMIP5, data_gfdl_CMIP5, file = "../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP5_weigthedMean_lastCheck.RData")

# # # for IPCC - Laurent Bopp
# save(data_ipsl_CMIP5, data_gfdl_CMIP5, file = "/Users/camillan/fishmip_outputs/CMIP6vsCMIP5_fig_temp/data_trends_CMIP5_IPCC_toCut.RData")
...
# # New folder structure - add DBEM in netcdf format (instead of RDS) - though still not final 
# # ADDED D to the below vectors - NO HISTORICAL FILE FOR IPSL...  also in function below date will need updating as per above models
# h_yearsD <- "1950_2014"
# f_yearsD<- "2015_2100"
# earthD<-"_ipsl-cm6a-lr_nobasd_"
# outputD<-"annual"
# extensionD<-".nc"
...
# ADDED [1] to yearmonth
combined<-extractFishCDF(directory2, filename , "Band1", yearmonth1[1], yearmonth2[1], average_whole_period = FALSE, CMIP = 6)
...
fut585<-extractFishCDF(directory2, filename , "Band1", yearmonth1[1], yearmonth2[1], average_whole_period = FALSE, CMIP = 6)
...
# NEW OutputData folder structure - remove DBPM and change earth
...
# 126
file<-readRDS(paste0(directory2, "EcoOcean CMIP6 RDS files/ecoocean_gfdl_nobc_ssp126_nosoc_default_tcb_global_monthly_2006_2100.RDS"))
fut126<-extractFishCDF_EcoOcean(file, type= "fut")

# 585
file<-readRDS(paste0(directory2,"EcoOcean CMIP6 RDS files/ecoocean_gfdl_nobc_ssp585_nosoc_default_tcb_global_monthly_2006_2100.RDS"))
fut585<-extractFishCDF_EcoOcean(file, type= "fut")
...
# save(data_ipsl_CMIP6, data_gfdl_CMIP6, file = "../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP6_lastCheck.RData")

# # file structure from OutputData folder
# save(data_ipsl_CMIP6, data_gfdl_CMIP6, file = "../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP6_OutputDataFolder.RData")
...
# load("../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP5_weigthedMean_lastCheck.RData")
# load("../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP6_lastCheck.RData")

# load("../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP6_OutputDataFolder.RData")
# load("../CMIP5vsCMIP6_data/temp_data/data_trends_CMIP6_wrong.RData")
```
...
mod_gfdl <- c("BOATS", "DBPM", "EcoOcean", "MACROECOLOGICAL") # new structure folder 
protocol <- c("picontrol","historical", "future")
future<-c("SSP126", "SSP585")
variable_to_extract <-c("tcb","tpb", "bp30cm", "bp30to90cm", "bp90cm")
...
a<-ggplot(filter(all, cmip==5), aes(group = protocol, x=year,y = mean, ymin = sd_min, ymax = sd_max))+
  ...
  geom_line(aes(color = protocol), size=0.3) +
  ...
  theme(text = element_text(size=7),
        axis.title.y = element_text(vjust=0.4, size = 7),
        axis.title.x = element_text(vjust=0.3, size = 7),
...
b<-ggplot(filter(all, cmip==6), aes(group = protocol, x=year, y = mean))+
  ...
  geom_line(aes(color = protocol),size=0.3) +
  geom_point(aes(y = A, color = protocol), size = 0.1)+
...
# NOTE: warning messages are OK - it is because cmip6 does not have data between 2006 and 2014 for ssp126 and ssp585 which are part of historical 

a<-a + ggtitle("a) CMIP5") + theme(plot.title = element_text(size = 7, face = "bold", hjust = 0))
b<-b + ggtitle("b) CMIP6") + theme(plot.title = element_text(size = 7, face = "bold",hjust = 0))
...
pdf(paste0(save_loc,name), height=2.8, width=7.086)
plot
dev.off()
...
# full set NEW run with updated DBPM 
#    cmip protocol  mean2   sd2
#   <dbl> <chr>     <dbl> <dbl>
# 1     5 ssp126    -4.41  3.18
# 2     5 ssp585   -15.5   9.60
# 3     6 ssp126    -5.90  2.95
# 4     6 ssp585   -17.2   5.92
...
# reduced set NEW run with updated DBPM 
#    cmip protocol  mean2   sd2
#   <dbl> <chr>     <dbl> <dbl>
# 1     5 ssp126    -4.54  3.30
# 2     5 ssp585   -16.2  10.1 
# 3     6 ssp126    -6.27  2.93
# 4     6 ssp585   -17.6   6.88
...
# 1 ssp126    2099     5  -4.78  2.97  -7.75  -1.81  -4.78
# 2 ssp126    2099     6  -6.75  2.88  -9.64  -3.87  -4.78
# 3 ssp585    2099     5 -16.5  10.1  -26.6   -6.48 -16.5 
# 4 ssp585    2099     6 -19.0   6.57 -25.5  -12.4  -16.5 
...
#   protocol  year  cmip   mean    sd sd_min sd_max      A
#   <chr>    <dbl> <dbl>  <dbl> <dbl>  <dbl>  <dbl>  <dbl>
# 1 ssp126    2099     5  -4.90  3.10  -8.00  -1.80  -4.90
# 2 ssp126    2099     6  -7.28  2.63  -9.91  -4.65  -4.90
# 3 ssp585    2099     5 -17.3  10.6  -27.9   -6.74 -17.3 
# 4 ssp585    2099     6 -19.3   7.59 -26.9  -11.7  -17.3 
...
length(test5_26$tcbChange) # 120
length(test5_85$tcbChange) # 120

# full set NEW run with updated DBPM 
# data:  test5_26$tcbChange and test5_85$tcbChange
# W = 12824, p-value < 2.2e-16
...
length(test6_26$tcbChange) # 160
length(test6_85$tcbChange) # 160

# full set NEW run with updated DBPM 
# data:  test6_26$tcbChange and test6_85$tcbChange
# W = 25105, p-value < 2.2e-16
# alternative hypothesis: true location shift is not equal to 0
...
a1<-a1 + ggtitle("a) CMIP5 IPSL-CM5A-LR") + theme(plot.title = element_text(size = 7, face = "bold", hjust = 0), axis.title.x = element_blank())
b1<-b1 + ggtitle("b) CMIP6 IPSL-CM6A-LR") + theme(plot.title = element_text(size = 7, face = "bold",hjust = 0), axis.title.x = element_blank(), axis.title.y = element_blank())
a2<-a2 + ggtitle("c) CMIP5 GFDL-ESM2M") + theme(plot.title = element_text(size = 7, face = "bold", hjust = 0))
b2<-b2 + ggtitle("d) CMIP6 GFDL-ESM4") + theme(plot.title = element_text(size = 7, face = "bold",hjust = 0), axis.title.y = element_blank()) # legend.position = "none"
...
# name = "Fig4_OutputDataFolder2.pdf"

# pdf(paste0(save_loc,name), height=12, width=15)
# plot
# dev.off()

pdf(paste0(save_loc,name), height=5.6, width=7.086)
plot
dev.off()
...
b1<-b1 + ggtitle("b) CMIP6 IPSL-CM6A-LR") + theme(plot.title = element_text(size = 7, face = "bold",hjust = 0), axis.title.x = element_blank(), axis.title.y = element_blank())
a2<-a2 + ggtitle("c) CMIP5 GFDL-ESM2M") + theme(plot.title = element_text(size = 7, face = "bold", hjust = 0))
b2<-b2 + ggtitle("d) CMIP6 GFDL-ESM4") + theme(plot.title = element_text(size = 7, face = "bold",hjust = 0), axis.title.y = element_blank()) 
...
# name = "FigS11_OutputDataFolder.pdf"
