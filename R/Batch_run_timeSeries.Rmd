...
EcoOcean netcdf in progress - need to add RDS files 
...
no error values in historical CMIP6

Derek comment about values being <0 in CMIP6 - see pdf 

thicker lines in CMIP6 when CMIP6 < CMIP5
...
# add EcoTrpoh IPSL ----
# no need for speceific function - the only thing is that the variable to extract is called "Band1" - you could adjust this in Batch_run_map.Rmd too 
...
file<-"EcoTroph/_tmp/ipsl-cm6a-lr/future/ecotroph_ipsl-cm6a-lr_nobc_ssp585_nat_default_tcb_annual_1950_2100.nc"
...
# # apply function - SOMETHING IS WORNG HERE! and BELOw so these files are not considered right now
# output[i] == "annual"
# 
# all<-get_timeSeries(hist, fut126, fut585)
# 
...
# 
# extractFishCDF_EcoOcean<-function(file, average_whole_period = FALSE, convert_to_kg_km = TRUE, type = "hist"){
#   
#   type = "hist"
#   
...
#   data_units <- "guess"
#     
#   # neeed lat and lon from somewhere 
...
# 
#   # extract all 
#   new_var_mat <- nc # ncvar_get(nc, variable)
#   years = time_vector
# 
...
# add EcoTrpoh GFDL and EcoOcean GFDL ----
...
b<-ggplot(filter(all, cmip==6), aes(group = protocol, x=year, y = mean))+
  ...
  geom_vline(xintercept = 2030, linetype = "dashed", color = "grey60", size = 1)+
  geom_hline(yintercept = 0, color = "grey60", size = 1)+
  scale_y_continuous(limits = c(-26, 12),name = "") +
  scale_x_continuous(limits = c(1970, 2100), breaks = pretty(all$year, n = 10), name = "Year")+
...
filter(all, year >2005, year <2010)
