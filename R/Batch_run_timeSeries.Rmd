...
a<-ggplot(filter(all, cmip==5), aes(group = protocol, x=year,y = mean, ymin = sd_min, ymax = sd_max))+
  ...
  # scale_color_manual(values = c("black" ,"red"), name = "Scenarios", labels = c("Historical", "SSP5-8.5"))+
  # scale_fill_manual(values = c("black" ,"red"), name = "Scenarios", labels = c("Historical", "SSP5-8.5"))+
  theme_bw()+
...
# # alternative for Derek presentation - only ssp585
# setwd("/Users/camillan/Desktop/")
# pdf("Fig3.pdf", height=6.5, width=15) # CONSIDER ALL MODELS
# plot
# dev.off()

if(set == "full"){
  pdf(paste0(save_loc,"Fig3.pdf"), height=6.5, width=15) # CONSIDER ALL MODELS
  plot
  dev.off()}else{
    pdf(paste0(save_loc,"SI/FigS7.pdf"), height=6.5, width=15) # CONSIDER LOTZE model set
    plot
    dev.off()
}
...
# FIG rends in tcb by model and esm and stat test 
...
# save data for more tests 
# save(all_models, file = "/Users/camillan/Desktop/trends_allModels.RData")
...
if(set == "full"){
  pdf(paste0(save_loc,"Fig4.pdf"), height=12, width=15) # CONSIDER ALL MODELS
  plot
  dev.off()
  }else{
    pdf(paste0(save_loc,"SI/FigS8.pdf"), height=12, width=15) # CONSIDER LOTZE MODEL SET
    plot
    dev.off()
  }
...
if(set == "full"){
  pdf(paste0(save_loc,"SI/FigS11.pdf"), height=12, width=15) # CONSIDER ALL MODELS
  plot
  dev.off()}else{
    pdf(paste0(save_loc,"SI/FigS17.pdf"), height=12, width=15) # CONSIDER LOTZE MODEL SET
    plot
    dev.off()  
}
