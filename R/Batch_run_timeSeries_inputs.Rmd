...
to_cmip5<-ggplot(filter(all_notMean$to, cmip==5), aes(x=year, y = tcbChange,  color = protocol, linetype = earth))+ 
  ...
  geom_vline(xintercept = 2030, linetype = "dashed", color = "grey60", size = 1)+
  geom_hline(yintercept = 0, color = "grey60", size = 1)+
  scale_y_continuous(limits = c(-1, 5), breaks = pretty(seq(5, -1), n = 5), name = "Increase in temperature (°C)") +
  scale_x_continuous(limits = c(1970, 2100), breaks = pretty(all$to$year, n = 10), name = "Year")+
...
to_cmip6<-ggplot(filter(all_notMean$to, cmip==6), aes(x=year, y = tcbChange,  color = protocol, linetype = earth))+ 
  ...
  theme(text = element_text(size=14),
        ...
        strip.text.x = element_text(face = "bold"),
        # legend.background=element_blank(),
        # legend.background= element_rect(colour = 'black', fill = 'white', linetype='solid'), # IF NEED legend contour
        legend.position = c(0.17, 0.7),
        legend.key.size = unit(0.6, "cm"))
...
to_cmip5<-to_cmip5 + ggtitle("A) CMIP5") + theme(plot.title = element_text(size = 16, face = "bold", hjust = 0), axis.title.x = element_blank())
to_cmip6<-to_cmip6 + ggtitle("B) CMIP6") + theme(plot.title = element_text(size = 16, face = "bold",hjust = 0), axis.title.x = element_blank(), axis.title.y = element_blank())
pp_cmip5<-pp_cmip5 + ggtitle("C) CMIP5") + theme(plot.title = element_text(size = 16, face = "bold", hjust = 0))
pp_cmip6<-pp_cmip6 + ggtitle("D) CMIP6") + theme(plot.title = element_text(size = 16, face = "bold",hjust = 0), axis.title.y = element_blank()) # legend.position = "none"

plot<-to_cmip5 + to_cmip6 + pp_cmip5 + pp_cmip6 +
  plot_layout(design = layout)

pdf(paste0(save_loc,"Fig1.pdf"), height=12, width=15)
plot 
dev.off()
