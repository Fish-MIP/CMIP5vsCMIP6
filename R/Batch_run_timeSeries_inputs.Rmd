...
# FIG 4B: trends in temperature and pp by earth system model and protocol
...
to_cmip5<-ggplot(filter(all_notMean$to, cmip==5), aes(x=year, y = tcbChange,  color = protocol, linetype = earth))+ 
  ...
  scale_y_continuous(limits = c(-5, 37), breaks = pretty(seq(37, -5), n = 10), name = "Change in temperature (%)") +
  scale_x_continuous(limits = c(1970, 2100), breaks = pretty(all$to$year, n = 10), name = "Year")+
  scale_color_manual(values = c("black","blue" ,"red"), name = "Protocols", labels = c("Historical", "RCP2.6", "RCP8.6"))+
  scale_fill_manual(values = c("black","blue" ,"red"), name = "Protocols", labels = c("Historical", "RCP2.6", "RCP8.6"))+
...
to_cmip5<-to_cmip5 + ggtitle("A) CMIP5") + theme(plot.title = element_text(size = 16, face = "bold", hjust = 0))
to_cmip6<-to_cmip6 + ggtitle("B) CMIP6") + theme(plot.title = element_text(size = 16, face = "bold",hjust = 0))

pp_cmip5<-pp_cmip5 + ggtitle(" ") + theme(plot.title = element_text(size = 16, face = "bold", hjust = 0))
pp_cmip6<-pp_cmip6 + ggtitle(" ") + theme(plot.title = element_text(size = 16, face = "bold",hjust = 0))

plot<-to_cmip5 + to_cmip6 + pp_cmip5 + pp_cmip6 +
  plot_layout(design = layout)

pdf(paste0(save_loc,"Fig4B.pdf"), height=12, width=15)
plot 
dev.off()
